...
static line_node_t *line_create_from_text(const char *text) {
    ...
    while (tok != NULL) {
        ...
        if (line->words == NULL) line->words = w;
        else tail->next = w;
        tail = w;
        tok = strtok(NULL, " \t");
    }
...
static int te_delete(const char *arg) {
    ...
    if (n == 1) {
        ...
        for (size_t i = 1; i < (size_t)n - 1; i++) prev = prev->next;
        victim = prev->next;
...
static int te_insert(const char *arg1, const char *arg2) {
    ...
    if (n == 1) {
        ...
        line_node_t *prev = global_state.lines;
        for (long i = 1; i < n - 1; i++) prev = prev->next;
    ...
    if (strcmp(cmd, "q") == 0) {
        ...
        if (argc > 1) {
            ...
            for (int i = 1; i < argc; i++) {
                ...
                if (i > 1 && used + 1 < sizeof(text)) text[used++] = ' ';
        ...
        if (argc > 2) {
            ...
            for (int i = 2; i < argc; i++) {
                size_t tok_len = strlen(argv[i]);
                if (i > 2 && used + 1 < sizeof(text)) text[used++] = ' ';
                if (used + tok_len < sizeof(text)) {
                    memcpy(text + used, argv[i], tok_len);
                    used += tok_len;
                }
